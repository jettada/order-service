...
export class OrderController {
  constructor(
    ...
    @Inject('INVENTORY_SERVICE') private inventortService: ClientProxy,
  ) {}

  @Post()
  async create(@Body() createOrderDto: CreateOrderDto) {
    const newOrder = await this.orderService.create(createOrderDto);
    console.log('sent process order');
    this.inventortService.emit('order_created', newOrder);
    return newOrder;
  }
  ...
  async handleOrderCancel(@Payload() data: any, @Ctx() context: RmqContext) {
